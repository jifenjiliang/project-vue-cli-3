...
util.queryData = function (options, fun) {
  ...
  let baseURL = ajaxconfig.baseURL
...
util.handleAjaxError = function ($vue, status, mess) {
  ...
  switch (status) {
    case '400':
      // token验证失败
      flag = true
      $vue.errorMess(mess ? mess : '参数错误')
      break
    case '403':
      // token验证失败
      flag = true
      $vue.errorMess('登录超时,请重新登录!')
      ...
      methods: {
        ...
        * */
        verifyAjaxResponse(response) {
          let flag = false
          if (this.$util._.isObject(response.data) || response.data.length) {
            flag = true
          }
          return flag
        },
        ...
        * */
        ...
        ajaxError(isLoadingFun) {
          ...
          return (error) => {
            if (error.response) {
              ...
              let message = error.response.data && error.response.data.errors[0].message ? error.response.data.errors[0].message : '';
              util.handleAjaxError(this, error.response.status + '', message)
        ...
        ajaxCreateLoading(flag) {
          if (listUrl === "/login/" || listUrl === "/buyer/checkOutValidateCode") return false
          if (flag) {
            // this.$Loading.start()
